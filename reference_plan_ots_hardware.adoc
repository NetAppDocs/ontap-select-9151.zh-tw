...
* VMware ESXi 8.0 U3 （適用於 ONTAP 9 。 15.1P4 或任何更新版本的可用修補程式版本）
...
如需詳細資訊，請參閱 https://mysupport.netapp.com/matrix["NetApp互通性對照表工具、window=_blank"]。
...
* 適用於伺服器的 Intel Xeon 處理器（如需詳細資訊，請參閱link:https://www.intel.com/content/www/us/en/products/processors/xeon/view-all.html?Processor+Type=1003["Intel Xeon處理器、window=_blank"]）
...
如需詳細資訊、請參閱 link:concept_lic_platforms.html["瞭解平台授權方案"] 。
...
* 記憶體72GB或更高、64 GB保留供ONTAP Select 不穩定使用
...
NOTE: 根據平台授權、還有額外的磁碟需求。如需詳細資訊、請參閱 link:reference_plan_ots_storage.html["儲存與RAID"] 。
