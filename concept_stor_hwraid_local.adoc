...
初始叢集建立可用於建立ONTAP Select 一個佔用初始資料存放區部分或全部空間的不實叢集。第二個步驟是使用其他資料存放區執行一或多個容量新增作業、直到達到所需的總容量為止。此功能詳述於一節link:concept_stor_capacity_inc.html["增加儲存容量"]。


NOTE: VMFS 的負荷不是零（請參閱link:https://kb.vmware.com/s/article/1001618["VMware知識庫1001618"]），而且試圖使用資料存放區回報為可用的整個空間，導致叢集建立作業期間發生假性錯誤。
...
NOTE: 將容量新增至ONTAP Select 某個VMware可能會導致不同大小的VMDK。有關詳細信息，請參閱一節link:concept_stor_capacity_inc.html["增加儲存容量"]。不同FAS 於VMware系統、不同大小的VMDK可存在於同一個集合體中。在這些VMDK上使用RAID 0等量磁碟區、無論其大小為何、都能充分利用每個VMDK中的所有空間。ONTAP Select
