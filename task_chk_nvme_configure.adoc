...
link:reference_chk_host_prep.html["主機準備檢查清單"]請遵循，檢閱，link:reference_chk_deploy_req_info.html["部署公用程式安裝所需的資訊"]和link:reference_chk_select_req_info.html["安裝所需的資訊ONTAP Select"]主題以取得更多資訊。
...
如需其他資訊、請參閱link:concept_stor_capacity_inc.html["增加儲存容量"]。
